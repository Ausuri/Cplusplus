...
// Project Number: 7b
// Project Desc: Hash Tables - Chained hashing
...
void Table::erase(int key)
{
   ...
   Node* ptr;
   assert( key >= 0 );
   findPtr( key, alreadyThere, ptr );
   if( alreadyThere )
   {
      int i = hash( key );      // get index of "bucket" where entry belongs
      // insert at beginning of list
      Node* temp = new Node;
      ptr->rec.key = -2;      // copy record
      ptr->rec.data = NULL;
      table[i] = temp;
      delete temp;
      delete ptr;
